...
function MovieCard({ item, series = true }) {
  ...
  return (
    <Link to={series ? `/series/${item.id}` : `/movie/${item.id}`}>
      <div className="movie-card flex flex-col gap-2 cursor-pointer">
        ...
        <p className="movie-card__title font-medium text-lg">
          {series ? item.name : item.title}
        </p>
        <p className="text-sm font-light">
          {(item.first_air_date || item.release_date) &&
            formatDate(series ? item.first_air_date : item.release_date)}
        </p>
      </div>
    </Link>
  );
